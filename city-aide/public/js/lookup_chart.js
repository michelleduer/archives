...
function broadbandLocation(results) {
    ...
    // PRINT VARIABLES
    // var locationID = '<p>stateFIPS: ' + stateFIPS + '<br>countyFIPS: ' + countyFIPS + '<br>geographyType: ' + geographyType + '</p>';
    
    // $("#broadbandDetails").html(locationID);
}
...
function providerDetails(results)
{
    ...
    var providerID = results.providerID;
    
    $.ajax({
        dataType: 'json',
        url: '/proxy.php?url=' + encodeURIComponent('http://www.broadbandmap.gov/broadbandmap/provider/dec2013/stats/state/' + stateFIPS + 
        '/population/county/' + countyFIPS + '/' + providerID + '?format=json'),
        success: function(data) {
            var provider = data.contents.Results;
            serviceDetails(provider.providerStatistics); 
        }
    });

    // PRINT VARIABLES
    //providers += '<p>' + counterProviders + ' Provider: ' + providerName + '<br>Provider #: ' + providerID + '</p>';
    
    counterProviders++;
        
    //$("#providerIDs").html(providers);
}
...
function serviceDetails(p)
{
    ...
    var meanDLSpeed = (p.mostCommonDownloadSpeedPercentage * 100).toFixed(0);
    var greatestDLSpeed = (p.greatestDownloadSpeedPercentage * 100).toFixed(0);
    ...
    {
        service += '<div class="col-sm-12"><h4>' + p.holdingCompanyName + '</h4></div><div class="col-sm-6"><p>Population: ' 
        + p.population + ', Households: ' + p.households + '<br><br><strong>DSL</strong>: ' + dsl + '<br><strong>Copper</strong>: ' + copper + 
        '<br><strong>Cable Modem</strong>: ' + cable + '<br><ul><li>Cable DOCSIS 3: ' + cableDocsis + '<br><li>Cable - Other: ' + cableOther + 
        '</li></ul><strong>Optical Fiber</strong>: ' + opticalFiber + '</p></div><div class="col-sm-6"><p>' + '<strong>Average Download Speed</strong>: ' + 
        p.mostCommonDownloadSpeed + '<br><strong>Average Download Speed Percentage</strong>: ' + meanDLSpeed + 
        '<br><strong>Greatest Download Speed</strong>: ' + p.greatestDownloadSpeed + '<br><strong>Greatest Download Speed Percentage</strong>: ' + 
        greatestDLSpeed + '<br><strong>Download Speeds</strong>:<br>6 Mbps+ : ' + mbps6 + '<br>10 Mbps+ : ' + mbps10 + '<br>25 Mbps+ : ' + 
        mbps25 + '<br>50 Mbps+ : ' + mbps50 + '<br>100 Mbps+ : ' + mbps100 + '<br>1 Gig+ : ' + gbps1 + '</p></div><div class="col-sm-12"><hr></div>';
...
function drawChart() {
    ...
    console.log("chart here", mbps6);
    var data = google.visualization.arrayToDataTable([
      ['Speeds (Mbps)', '% of Customers', { role: 'style' }],
    ...
    var options = {
      title: 'Average Consumer Speeds',
      vAxis: {title: 'Speeds (Mbps)',  titleTextStyle: {color: 'red'}}
    };
...
function dataGreatSchools(options, callback) {
    $.ajax({
        dataType: 'json',
        url: '/proxy.php?url=' + encodeURIComponent('http://api.greatschools.org/schools/nearby?key=hn2xcynxteqjtz7op24exwxx&state=' + options.state + '&zip=' + options.zip),
        success: function(data) {
